      ...
      <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-9 gap-2">
        {AVATAR_LIST.map((avatar) => (
          <button
            ...
            className={`relative overflow-hidden rounded-lg border-2 transition-all hover:scale-105 aspect-square ${
              value === avatar.id
          ...
          >
            ...
            {value === avatar.id && (
              <div className="absolute inset-0 bg-accent/20 flex items-center justify-center">
                <Check className="h-4 w-4 md:h-5 md:w-5 text-accent drop-shadow-lg" />
              </div>
            )}
          </button>
        ))}
